        ...
        <div className="p-6">
            ...
            <TableShell page={applications} columns={['Applicant', 'Profile', 'Pipeline', 'Action']} render={(app: any) => (
                <>
                    ...
                    <td className="px-4 py-3"><div>{app.user?.employee_profile?.headline}</div><div className="text-muted-foreground">{app.user?.employee_profile?.experience_years ?? 0} years experience</div>{app.cv_file && <a className="text-emerald-700" href={`/storage/${app.cv_file}`}>CV</a>}<div className="mt-1 text-xs text-muted-foreground">{app.user?.employee_profile?.skills?.map((skill: any) => skill.name).join(', ')}</div><div className="mt-2 rounded-md border px-2 py-1 text-xs"><span className="font-medium">{app.match_score ?? 0}% match</span><div className="text-muted-foreground">{(app.match_reasons ?? []).join(' - ')}</div></div></td>
                    ...
                    <td className="px-4 py-3">
                        ...
                        <div className="mt-3 grid gap-2">
                            ...
                            <Form action={`/employer/applications/${app.id}/messages`} method="post" className="grid gap-2">
                                <textarea name="body" placeholder="Message applicant" className="min-h-16 rounded-md border bg-background px-2 py-1 text-sm" />
